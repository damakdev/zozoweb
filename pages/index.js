...
import {
	_getAllCompletedEvents,
	_getAllOngoingEvents,
	_getAllUpcomingEvents,
	_getApprovedBiddingEvents,
...
function Home() {
	...
	const ongoing = useSelector((state) => state.events.allOngoingBids);
	const upcoming = useSelector((state) => state.events.allUpcomingBids);
	const completed = useSelector((state) => state.events.allCompletedBids);
...
seek win.`,
	...
	useEffect(() => {
		dispatch(_getAllOngoingEvents({ status: "ongoing" }));
		dispatch(_getAllUpcomingEvents({ status: "upcoming" }));
		dispatch(_getAllCompletedEvents({ status: "completed" }));
			...
			<CustomerLayout>
				...
				<section className={styles.body}>
					<div className={styles.sidebar}>
						<BidInfoCard data={ongoing.events} title="Ongoing Bids" />
						<BidInfoCard data={upcoming.events} title="Upcoming Bids" />
					...
					<div className={styles.content}>
						...
						{ongoing.events.length > 0 && (
							<ProductsSection products={ongoing.events} title="Ongoing Bids" />
						)}

						{completed.events.length > 0 && (
							<ProductsSection
								products={completed.events}
