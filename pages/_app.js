...
function MyApp({ Component, pageProps, router }) {
	...
	axios.interceptors.request.use(
		function (config) {
			// console.log(merchanToken)
			if (localStorage.getItem("persist:zozo")) {
				const { admin, customer, merchant } = JSON.parse(
					localStorage.getItem("persist:zozo")
				);
			...
			if (adminToken || merchanToken !== undefined || customerToken) {
				if (url[1] == "admin")
					config.headers.Authorization = `Bearer ${adminToken}`;
				if (url[1] == "merchant")
					config.headers.Authorization = `Bearer ${merchanToken}`;
				if (url[1] == "customer")
