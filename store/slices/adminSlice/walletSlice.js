...
export const _getMerchantWallets = createAsyncThunk(
	"wallet/getMerchantWallets",
	async () => {
		return await getMerchantWallets();
	}
);

export const _getWithdrawRequest = createAsyncThunk(
	"wallet/getWithdrawRequest",
	async (status) => {
...
let walletSlice = createSlice({
	...
	extraReducers: {
		...
		[_getMerchantWallets.rejected]: (state, action) => {
			state.walletList.isLoading = false;
		},
		...
		[_getWithdrawRequest.rejected]: (state, action) => {
			state.withdrawalRequest.isLoading = false;
		},
	},
});
